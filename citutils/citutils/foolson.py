...
1. Indentation instead of curly braces. For simplicity, indentation is centered around "indentons", which each are 2 single-space characters (ASCII 0x20, U+0020), like so:  . An "indentation" at the beginning of a line is made up of one or more indentons. The key concept of indentation is that one greater level of indentation indicates the interior of a json object. Note that this means the KEYS are the thing we're primarily concerned with getting indented; indenting the values is just a side-effect (this is different from some other formats). Note that this means that to have a file of key-value pairs, the entire file must be indented by one.
2. quotes are unnecessary (TODO: this feature currently pending discussion)
